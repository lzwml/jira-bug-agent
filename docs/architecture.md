...
CLI / future Web UI
        │
        ▼
BugAnalysisAgent        Agent Loop、步骤预算、状态、Prompt
        │
...
`BugAnalysisAgent` 不知道 Jira REST URL、Log MCP 文件路径或模型厂商 SDK。它只
依赖两个小接口：模型完成一次对话、工具路由执行一次调用。因此 Provider 和 MCP
都可以替换，而 Agent Loop、领域 Prompt 和运行状态保持稳定。
