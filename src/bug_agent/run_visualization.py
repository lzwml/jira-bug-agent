...
const metricData=[['状态',result.status||bundle.lifecycle?.status],['分析工具',agentMetrics.analysis_tool_calls??actual.tool_calls??0],['人工检查点',agentMetrics.human_checkpoint_count??0],['重复调用',agentMetrics.repeated_identical_calls??0],['证据',derived.evidence_registry?.length??0],['耗时',actual.duration_ms==null?'未知':actual.duration_ms+' ms']];
