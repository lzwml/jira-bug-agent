...
const result=bundle.result||{}, report=result.report||{}, derived=bundle.derived||{}, budget=bundle.budget||{}, actual=budget.actual||{}, provenance=bundle.provenance||{}, fp=derived.input_fingerprint||{};
document.getElementById('subtitle').textContent=`${bundle.task?.task_id||''} · ${bundle.task?.objective||''}`;
const metricData=[['状态',result.status||bundle.lifecycle?.status],['步骤',actual.steps??0],['工具调用',actual.tool_calls??0],['证据',derived.evidence_registry?.length??0],['耗时',actual.duration_ms==null?'未知':actual.duration_ms+' ms']];
