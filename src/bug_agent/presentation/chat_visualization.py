...
<aside class="pane"><div class="pane-head"><h2>调查轮次 <span class="turn-counter" id="turn-counter"></span></h2><p>选择一轮，右侧内容会同步切换。</p><input class="round-search" id="round-search" type="search" placeholder="搜索人工输入、回答或工具"></div><div class="pane-scroll round-list" id="round-list"></div></aside>
<section class="pane"><div class="pane-head"><h2 id="selected-title">会话内容</h2><p id="selected-summary">人工输入与 Agent 回答保持在同一个阅读面。</p></div><div class="pane-scroll" id="conversation-panels"></div></section>
<aside class="pane"><div class="pane-head"><div class="row"><div><h2>本轮调查</h2><p>核对工具，并记录该轮暴露的 Agent 问题。</p></div><button id="export">导出复盘</button></div><div class="right-tabs"><button class="right-tab active" data-right="tools">工具与返回</button><button class="right-tab" data-right="optimization">优化标注</button></div></div><div class="pane-scroll"><div class="right-panel active" id="tools-panels"></div><div class="right-panel" id="optimization-panels"></div></div></aside>
</div>'''
_WORKSPACE_JS = r'''
const roundList=document.getElementById('round-list'),conversationPanels=document.getElementById('conversation-panels'),toolsPanels=document.getElementById('tools-panels'),optimizationPanels=document.getElementById('optimization-panels');
const interactionOptions=[['task_definition','任务设定'],['context_supply','补充上下文'],['scope_refinement','调整调查范围'],['evidence_direction','指定证据方向'],['hypothesis_challenge','挑战已有假设'],['fact_correction','纠正事实或概念'],['request_deeper','要求继续深挖'],['confirmation','确认当前结论']];
const targetOptions=[['tool_selection','工具选择或参数'],['skill_routing','Skill 路由'],['skill_content','Skill 内容'],['case_context','Case 输入'],['unknown','暂不确定']];
function addOptions(select,options){options.forEach(([value,label])=>{const option=el('option','',label);option.value=value;select.append(option)})}
function interactionDefault(t,index){if(index===0)return'task_definition';if(t.human_intervention?.kind)return t.human_intervention.kind;return(t.tool_events||[]).length?'evidence_direction':'request_deeper'}
function targetDefault(t){return t.human_intervention?.attribution?.target||((t.tool_events||[]).some(x=>x.tool_name==='activate_skill')?'skill_routing':(t.tool_events||[]).length?'tool_selection':'unknown')}
turns.forEach((t,index)=>{
 const events=t.tool_events||[],item=el('button','round-item');item.type='button';item.dataset.turn=String(t.turn_index);item.dataset.search=[t.user_message,t.assistant_answer,...events.map(x=>x.tool_name)].join('\n').toLowerCase();const top=el('span','round-item-top');top.append(el('strong','',`第 ${t.turn_index} 轮`),el('span','round-meta',events.length?`${events.length} 次工具调用`:index===0?'任务设定':'人工参与'));item.append(top,el('span','round-preview',(t.user_message||'（无人工输入）').replace(/\s+/g,' ')));item.onclick=()=>selectTurn(t.turn_index);roundList.append(item);
 const conversation=el('div','turn-panel');conversation.dataset.turn=String(t.turn_index);const human=el('section','conversation-section'),agent=el('section','conversation-section');human.append(el('h3','human-title','人工输入'),markdown(t._user_html));agent.append(el('h3','agent-title','Agent 回答'),markdown(t._assistant_html));conversation.append(human,agent);conversationPanels.append(conversation);
 const tools=el('div','turn-panel');tools.dataset.turn=String(t.turn_index);if(!events.length)tools.append(el('div','empty','本轮没有调用工具。'));events.forEach((x,eventIndex)=>{const tool=el('details','tool-card right-tool');tool.open=eventIndex===0;const heading=el('summary');heading.append(el('strong','',x.tool_name||'unknown'),el('span','preview',toolDef(t,x.tool_name).description),el('span',`tag ${x.success?'ok':'bad'}`,x.success?'成功':'失败'));tool.append(heading);const content=el('div','body');appendToolDetails(content,t,x,true);const review=content.querySelector('.review');if(review)review.dataset.name=x.tool_name||'unknown';tool.append(content);tools.append(tool)});toolsPanels.append(tools);
 const optimization=el('div','turn-panel workspace-review');optimization.dataset.turn=String(t.turn_index);const checkWrap=el('label','check'),check=document.createElement('input');check.type='checkbox';check.className='mark';check.checked=index>0;checkWrap.append(check,document.createTextNode(index===0?' 将任务设定纳入优化样本':' 将本轮人工输入纳入 Agent 优化样本'));optimization.append(checkWrap);
 const kindWrap=el('div'),kindLabel=el('label','','人工在这一轮做了什么'),kind=document.createElement('select');kind.className='kind';addOptions(kind,interactionOptions);kind.value=interactionDefault(t,index);kindWrap.append(kindLabel,kind,el('div','field-help','用于区分补充证据、收窄范围、纠正事实等不同类型的人工参与。'));
 const targetWrap=el('div'),targetLabel=el('label','','Agent 的哪一部分需要优化'),target=document.createElement('select');target.className='target';addOptions(target,targetOptions);target.value=targetDefault(t);targetWrap.append(targetLabel,target,el('div','field-help','重点判断是工具选择/参数、Skill 路由、Skill 内容，还是 Case 输入。'));
 const noteWrap=el('div'),noteLabel=el('label','','具体暴露了什么问题（可选）'),note=document.createElement('input');note.type='text';note.className='note';note.placeholder='例如：inspect_case 返回了附件名，但没有展示可核对路径';noteWrap.append(noteLabel,note);optimization.append(kindWrap,targetWrap,noteWrap);optimizationPanels.append(optimization);
});
let selectedTurn=null;function selectTurn(turnIndex){selectedTurn=String(turnIndex);document.querySelectorAll('.round-item').forEach(x=>x.classList.toggle('active',x.dataset.turn===selectedTurn));document.querySelectorAll('.turn-panel').forEach(x=>x.classList.toggle('hidden',x.dataset.turn!==selectedTurn));const t=turns.find(x=>String(x.turn_index)===selectedTurn);document.getElementById('selected-title').textContent=`第 ${t?.turn_index||''} 轮 · 人工与 Agent`;document.getElementById('selected-summary').textContent=`${(t?.tool_events||[]).length} 次工具调用 · ${t?.agent_status||'状态未记录'}`;location.hash=`turn-${selectedTurn}`}
document.getElementById('turn-counter').textContent=`${turns.length} 轮`;document.getElementById('round-search').oninput=e=>{const query=e.target.value.trim().toLowerCase();document.querySelectorAll('.round-item').forEach(x=>x.classList.toggle('hidden',query&&!x.dataset.search.includes(query)))};
document.querySelectorAll('.right-tab').forEach(tab=>tab.onclick=()=>{document.querySelectorAll('.right-tab').forEach(x=>x.classList.toggle('active',x===tab));document.querySelectorAll('.right-panel').forEach(x=>x.classList.toggle('active',x.id===`${tab.dataset.right}-panels`))});
function exportReview(){const annotations=[...document.querySelectorAll('.workspace-review')].map(card=>{if(!card.querySelector('.mark').checked)return null;const t=turns.find(x=>String(x.turn_index)===card.dataset.turn);return{turn_index:t.turn_index,interaction_kind:card.querySelector('.kind').value,target:card.querySelector('.target').value,note:card.querySelector('.note').value.trim(),human_feedback:t.human_intervention?.response||t.user_message,structured_intervention:!!t.human_intervention,subsequent_tools:(t.tool_events||[]).map(x=>x.tool_name)}}).filter(Boolean);const tool_reviews=[...document.querySelectorAll('.right-tool .review[data-step]')].map(card=>{const verdict=card.querySelector('.tool-verdict').value;if(!verdict)return null;return{turn_index:Number(card.dataset.turn),step:Number(card.dataset.step),tool_call_id:card.dataset.call||null,tool_name:card.dataset.name||'',verdict,note:card.querySelector('.tool-note').value.trim()}}).filter(Boolean);const output={schema_version:2,record_kind:'chat_review',session_id:session.session_id,source_updated_at:session.updated_at,annotations,tool_reviews};const blob=new Blob([JSON.stringify(output,null,2)],{type:'application/json'}),link=document.createElement('a');link.href=URL.createObjectURL(blob);link.download=`chat-review-${session.session_id||'session'}.json`;link.click();URL.revokeObjectURL(link.href)}
document.getElementById('export').onclick=exportReview;const hashTurn=location.hash.match(/^#turn-(\d+)$/)?.[1],initial=turns.some(t=>String(t.turn_index)===hashTurn)?hashTurn:turns.at(-1)?.turn_index;if(initial!==undefined)selectTurn(initial);
'''
...
_SITE_PAGES = (
    ("index.html", "Agent 三栏复盘", "workspace", _WORKSPACE_BODY, _WORKSPACE_JS),
...
const usageItems=turns.map(t=>t.token_usage).filter(Boolean),totalTokens=usageItems.reduce((n,x)=>n+(x.total_tokens||0),0),tokenLabel=usageItems.length?String(totalTokens)+(usageItems.some(x=>x.complete===false)?'（部分）':''):'未知';[['人工输入',turns.length],['后续调查动作',allEvents.length],['使用工具',usedTools.length],['Token',tokenLabel],['显式纠错',turns.filter(isExplicitCorrection).length]].forEach(([k,v])=>{const box=el('div','metric');box.append(el('b','',v),el('span','',k));document.getElementById('metrics').append(box)});
